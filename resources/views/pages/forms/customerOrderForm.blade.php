...
<html lang="en">
    ...
    <main class="container mb-5 py-5 min-vh-100">
        ...
        <div class="row view" id="order-dump" style="display: none;">
            ...
            <div class="row">
                <div class="col-md-12 py-3 text-center">
                    <div class="display-6">{{ $orderLabel->orderbump_heading }}</div>
                    <div class="fw-bold lead">{{ $orderLabel->orderbump_subheading }}</div>
                </div>
            </div>
                ...
                <aside class="col-lg-6">
                    <article class="gallery-wrap gallery-vertical">
                        <a href="#" class="img-big-wrap img-thumbnail bg-light">
                            <img height="520" src="{{ asset('/storage/products/'.$orderbump_outgoingStock->product->image) }}" class="mix-blend-multiply">
                        </a> <!-- img-big-wrap.// --> 
                    </article> <!-- gallery-wrap .end// --> 
                </aside> 
                <div class="col-lg-6"> 
                    <article class="ps-lg-3"> 
                        <h4 class="title text-dark">{{ $orderbump_outgoingStock->product->name }}</h4> 
                        <hr> 
                        <ul class="list-check cols-two mb-4">
                            @if ($features_orderbump !== '')
                                @foreach ($features_orderbump as $features)
                                    <li>{{ $features }} </li>
                                @endforeach
                            @endif 
                        ...
                        <div class="mb-4"> 
                            <var class="price h5">
                                <span class="currency"></span>{{ $orderbump_outgoingStock->product->price }}
                            </var> 
                            <span>per 1 </span> 
                        </div> <!-- price-wrap.// --> 
                        <div class="row gx-2 mb-4"> 
                            ...
                            <div class="col-auto orderbump_add_to_order"
                                data-orderbump_revenue={{ $orderbump_outgoingStock->product->price * $orderbump_outgoingStock->quantity_removed }}> 
                                <button type="button" class="btn btn-primary w-100"><i class="me-2 fa fa-shopping-basket"></i> Add to Order </button> 
                            </div> <!-- col.// --> 
                            
                        </div> <!-- row.// --> 
                        <div class="row">
                            <div class="col-auto text-dark fw-bolder">
                                (<span class="product_quantity">{{ $orderbump_outgoingStock->quantity_removed }}</span>
                                pieces - <span class="product_amount">
                                <!---unitprice * qty_removed---->
                                <span class="currency"></span>{{ $orderbump_outgoingStock->product->price * $orderbump_outgoingStock->quantity_removed }}</span>)
                            </div> <!-- col.// --> 
                        </div>
                        <input type="hidden" name="orderbump_revenue_store" class="orderbump_revenue_store" value="">
                    </article> 
                </div> <!-- col.// -->
            </div> <!-- row.// -->
        </div>
        ...
        <div class="row view" id="upsell" style="display: none;">
            ...
            <div class="row">
                <div class="col-md-12 py-3 text-center">
                        <div class="display-6">{{ $orderLabel->upsell_heading }}</div>
                        <div class="fw-bold lead">{{ $orderLabel->upsell_subheading }}</div>
                    </div>
                </div>
                ...
                <div class="row g-0">
                    <aside class="col-xl-3 col-lg-4 col-md-12 col-12">
                        <a href="#" class="img-wrap"> 
                            <img src="{{ asset('/storage/products/'.$upsell_outgoingStock->product->image) }}">
                        </a>
                    </aside> <!-- col.// --> 
                    <div class="col-xl-6 col-lg-5 col-md-7 col-sm-7">
                        <div class="card-body">
                            <a href="#" class="h6 title mb-3">{{ $upsell_outgoingStock->product->name }} </a> 
                            <ul class="list-check mb-2"> 
                                @if ($features_upsell !== '')
                                @foreach ($features_upsell as $features)
                                    <li>{{ $features }} </li>
                                @endforeach
                            @endif 
                            </ul> 
                    ...
                    <aside class="col-xl-3 col-lg-3 col-md-5 col-sm-5">
                        <div class="info-aside"> 
                            <div class="price-wrap"> 
                                <span class="price h5"><span class="currency"></span>{{ $upsell_outgoingStock->product->price }} per one</span> 
                                {{-- <del class="price-old"> <span class="currency"></span>9980</del>  --}}
                            </div> <!-- info-price-detail // --> 
                            <p>
                                <i class="bi bi-check-circle-fill text-success"></i>
                                (<span class="product_quantity">{{ $upsell_outgoingStock->quantity_removed }}</span>
                                pieces - <span class="product_amount">
                                <!---unitprice * qty_removed---->
                                <span class="currency"></span>{{ $upsell_outgoingStock->product->price * $upsell_outgoingStock->quantity_removed }}</span>)
                            </p>
                            <br>
                            {{-- <div class="mb-2">
                                <div class="input-group input-spinner">
                            ...
                            <button class="btn btn-primary w-100 mb-2 upsell_add_to_order" data-upsell_revenue={{ $upsell_outgoingStock->product->price * $upsell_outgoingStock->quantity_removed }}>
                                <i class="me-2 fa fa-shopping-basket"></i>
                                Add to Order
                            </button>
                            <input type="hidden" name="upsell_revenue_store" class="upsell_revenue_store" value=""> 
                        </div> <!-- info-aside.// --> 
                    </aside> <!-- col.// --> 
                </div> <!-- row.// -->
            </article>
...
$(".orderbump_add_to_order").click(function(){
    var mainProduct_revenue = $('.mainProduct_revenue').val();
    var orderbump_revenue = $(this).attr('data-orderbump_revenue');
    $('.orderbump_revenue_store').val(orderbump_revenue);

    var subtotal = parseFloat(mainProduct_revenue) + parseFloat(orderbump_revenue);
    $('.revenue_subtotal').text(subtotal);
    $('.grand_total').val(subtotal);
});

$(".upsell_add_to_order").click(function(){
    var mainProduct_revenue = $('.mainProduct_revenue').val();
    var upsell_revenue = $(this).attr('data-upsell_revenue');
    $('.upsell_revenue_store').val(upsell_revenue);

    var orderbump_revenue_store = $('.orderbump_revenue_store').val();
    
    if ( orderbump_revenue_store !== '') {
        var subtotal = parseFloat(mainProduct_revenue) + parseFloat(orderbump_revenue_store) + parseFloat(upsell_revenue);
        $('.revenue_subtotal').text(subtotal);
        $('.grand_total').val(subtotal);
    } else {
        var subtotal = parseFloat(mainProduct_revenue) + parseFloat(upsell_revenue);
        $('.revenue_subtotal').text(subtotal);
        $('.grand_total').val(subtotal);
    }
...
// phone.addEventListener('change', checkEnableButton)
...
// address.addEventListener('change', checkEnableButton)
// country.addEventListener('change', checkEnableButton)


// $(".completeBtnDiv").click(function(){

//     var grand_total = $('.grand_total').val();
//     var unique_key = $('.unique_key').val();
//     $.ajax({
// 			type:'get',
// 			url:'/complete-customer-order',
// 			data:{grand_total:grand_total, unique_key:unique_key},
// 			success:function(resp){
//                 console.log(resp.unique_key)
    ...
    const completeStep = () => {
        document.getElementById('completeBtn').addEventListener('click',()=>{ 
            ...
            $.ajax({
                type:'get',
                url:'/complete-customer-order',
                data:{grand_total:grand_total, unique_key:unique_key, orderbump_revenue:orderbump_revenue, upsell_revenue:upsell_revenue,
                    firstname:firstname, lastname:lastname, phone:phone, email:email, city:city, state:state, country:country, address:address
                    },
                success:function(resp){
                    console.log(resp)
